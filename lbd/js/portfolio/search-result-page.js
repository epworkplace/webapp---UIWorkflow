...
//        $("#langauge").multiselect().multiselectfilter();
    ...
    $("#con_status").chosen({ allow_single_deselect: true,search_contains: true });
    $("#status").chosen({ allow_single_deselect: true,search_contains: true });
    $("#status2").chosen({ allow_single_deselect: true,search_contains: true });
    ...
    $("#sites").chosen({ allow_single_deselect: true,search_contains: true });
...
/*... Added on 3-08-2016 Reason: export portfolio ...*/
function exportPortfolio(){
    $("#export_portfolio").html('Loading...');
    var form_data = $("#main_form").serialize()+'&'+$("#filter_form").serialize();
    $.ajax({
        type: 'POST',
        url: '/portfolio/export-portfolio?'+form_data,
        success: function (data) {
            data = $.parseJSON(data);
            if(data.actions == 'alert'){
                alert('oops.. Zero result to export.');
            }
            else if(data.actions == 'download'){
                location.href ="/BO/download-files.php?function=downloadFileAndDelete&delete=yes&fullPath="+data.file;
            }else {
                var w = window.open();
                $(w.document.body).html(data.content);
            }
			$("#export_portfolio").html('Export Portfolio');
            //console.log(data);     
        } 
    });
}
/*... End Added on 3-08-2016 ...*/